...
use crate::core::{Value, Object, SapfError};
...
fn sf_read_prim(thread: &mut Thread, _prim: &Primitive) -> std::result::Result<(), SapfError> {
    // Pop frames, offset, filename from stack
    let frames = thread.pop_real("sf_read frames")?;
    let offset = thread.pop_real("sf_read offset")?;
...
fn sf_write_prim(thread: &mut Thread, _prim: &Primitive) -> std::result::Result<(), SapfError> {
...
///
/// Stack effect: data filename -> 
fn sf_write_open_prim(thread: &mut Thread, _prim: &Primitive) -> std::result::Result<(), SapfError> {
    // Pop filename and data from stack
    let filename = thread.pop()?;
    let data = thread.pop()?;
...
pub fn register_sound_file_builtins(vm: &crate::vm::VM) -> std::result::Result<(), SapfError> {
    use std::sync::Arc;
